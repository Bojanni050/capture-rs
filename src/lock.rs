//! Voorkomt dat twee `capture start`-processen tegelijk naar dezelfde
...
//! `capture start`-instanties). Een gewoon PID-bestand zou na een crash of
        ...
        OpenOptions::new()
            ...
            .map_err(|e| {
                if e.raw_os_error() == Some(ERROR_SHARING_VIOLATION) {
                    anyhow!("er draait al een andere opname (capture start) — sluit die eerst af")
                } else {
